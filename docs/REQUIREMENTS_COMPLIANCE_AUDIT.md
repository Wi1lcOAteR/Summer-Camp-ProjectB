...
| Architecture/data flow/data model | SPEC 6--7 | core and learning schema plus M1/M2/M3 repositories implemented; app factory and M1 routes at `6d44eac` | API-02--03 |
...
| TDD red-green-refactor | exact red/green command in all 39 cards; shared assertion-level 2--5 minute step expansion | verified through API-01, including missing app RED and preserved-filename RED | continue every behavior task |
| SPEC review then quality/security/license review | terminal hash-bound review loop | verified through API-01; loopback trust, CSRF, upload limits, stable errors, M1 flow and static/private containment reviewed | every-task protocol; Critical stops sequence |
...
| `AGENT_LOG.md` timestamp/task/skill/context/output/hash/human edits/lesson | historical and current entries exist | current through API-01 | coordinator evidence commit after every task |
