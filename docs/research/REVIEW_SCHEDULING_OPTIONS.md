...
### `ReviewPolicy v1` 精确合同（已随整体 SPEC 确认）
...
唯一实现语义以最新 `SPEC.md` 的 `plan_reviews_v1` 纯函数、local-date/DST 规则、UUIDv5 输入哈希和 G-01 至 G-05 fixtures 为准；本文摘要不能被 PLAN 用来另写一套装箱/到期算法。该规则已随整体签字成为 v1 合同，但不宣称科学最优。
...
5. D-020 的保守确定性方向及最新 `SPEC.md` 提出的 v1 数值/纯函数已随整体 SPEC 确认；PLAN 只能落实并测试，仍须提供实现与学习效果证据。
