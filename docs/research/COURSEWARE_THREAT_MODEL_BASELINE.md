...
本文最初是在 D-008 尚未确认时形成的方案无关基线；D-014 现已确认第一版实现 L/P/F 三种能力，但不预设具体供应商、模型或数据政策。

后续学生反馈确认：纯本地解析可能损失公式、代码、图表或版面语义，因此产品应允许用户选择处理路径。第一版提供 L/P/F，但没有隐式默认外发；每次实际文件范围仍需授权。该选择不会取消本文件中的数据最小化、外发可见性和凭据保护要求；供应商与数据政策仍未确认。
...
| T-02 | 课件在用户不知情时上传 | 无政策/文件级 consent 时上传为 0；P 展示页面/片段，F 展示整份文件清单、大小、provider 政策与成本并要求明确动作 | mock 供应商断言无确认时调用次数为 0；请求范围与 consent 精确相等 |
...
| T-18 | 整份上传/索引部分失败或删除失败造成孤儿远端对象 | 每文件 `RemoteMaterialObject` 与 `RemoteJob` 状态、幂等键、provider 引用和删除对账；未知状态禁止新检索并显示 `delete_incomplete` | 重启/取消/部分失败/离线恢复后对象仍可追踪；删除未确认不显示成功 |
...
- D-008/D-014：能力目录已确认 L/P/F；供应商、部署区域、留存与训练政策仍未决；
