...
D-014 已确认第一版实现整份 PDF/课程远端处理（模式 F），并由用户决定是否使用。本文件定义 provider 无关的授权、上传、索引、使用、切换和删除合同；不选择 provider、SDK、模型、区域、留存政策、费用或凭据存储实现。
...
| `retention_snapshot_id` | 上传时展示的数据政策/保留期快照 |
...
- provider、目的、文件名/内部 ID、数量、总大小和内容哈希；
...
`ConsentRecord.payload_scope` 必须精确包含本批文件/哈希和 provider。更换文件内容、追加文件、更换 provider、从 L/P 切到 F 或撤销后重新启用都需要新记录；不能复用更宽的旧授权。
...
1. 校验 consent、文件哈希、类型、大小、配额和 provider 能力快照。
2. 使用 `(provider, material_id, content_hash, consent_record_id)` 生成幂等键。
...
若适配器不能提供上传后的对象追踪或任何删除/过期语义，则不能启用模式 F；不得仅凭模型能接收文件就宣称满足本合同。
...
- 具体 provider、SDK、模型、区域、数据政策和费用上限；
