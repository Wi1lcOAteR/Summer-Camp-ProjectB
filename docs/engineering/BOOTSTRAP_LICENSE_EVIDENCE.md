...
F-01B must fetch only the immutable commit URLs, verify byte count and SHA-256 before copying, and fail closed on any mismatch. The release artifact hashes consumed by F-01A from `DEPENDENCY_BASELINE.md` remain separate mandatory checks.
