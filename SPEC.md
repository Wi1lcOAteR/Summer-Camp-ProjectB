# ProjectB 精简 v1 规约（待学生重新确认）

> **状态：阶段 A 范围收束稿，尚未签字。** 学生已于 2026-07-25 选择：精简纵向切片、模型范围 `L+P`、数字 PDF + UTF-8 文本/Markdown、通用知识点模型并以互斥/竞态/死锁三个概念验收、使用 Claude Code 做后续冷启动。本文取代 2026-07-20 的旧 v1 文本；旧签字不自动覆盖本稿。
>
> **实现门禁：** 本文经学生整体确认后，才能用 `superpowers:writing-plans` 重写 `PLAN.md`。完整计划、不同类型智能体冷启动、修订和学生实现批准完成前，禁止创建正式实现源码。
...
| 风险/决策 | 当前处理 |
| --- | --- |
| 本稿尚未签字 | 当前最近硬门禁；未确认前不生成正式实现 PLAN |
