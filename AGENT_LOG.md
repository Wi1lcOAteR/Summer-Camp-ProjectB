...
## 2026-08-12T22:20:00+08:00 - Review workflow release closure and README rewrite

- **Trigger / scope:** manual Edge acceptance showed that the previous Review page displayed hard-coded fixture data and its primary buttons did not call the backend. The student requested a submission-grade usable closure, a push, and a complete README rewrite for the same-night deadline. Work stayed within the existing review API/client/UI/tests and delivery documentation; no dependency or architecture change was introduced.
- **RED:** replacement ReviewView tests failed against the old page because it never loaded a backend revision and never persisted complete/skip/recover. The stale browser contract also referenced placeholder file names, hashes, and cosmetic-only controls.
- **GREEN:** the API client now parses real review revisions/tasks and calls generate, complete, skip, and recover endpoints. ReviewView loads the current course/concepts, displays the persisted revision, starts the first pending task, and persists task state changes. Focused Vitest returned `3 passed`; the complete frontend gate returned `8 files passed, 59 tests passed`, TypeScript unused checks passed, and Vite production build passed.
- **Artifact and real UI:** pinned PyInstaller `6.21.0` produced `WINDOWS_BUILD_PASS`; final `ProjectB.exe` is `29,225,145` bytes with SHA-256 `CB463E179FE2D3367ED0BA96B0621A5E32CF01319F39DB0AB74FC9FCC237F6A3`. Disposable-data smoke returned `WINDOWS_SMOKE_PASS`. The rebuilt app generated a real five-task revision for the retained local course. Browser acceptance then executed start, skip, and recover; the final state returned to five pending and zero skipped tasks, proving persistence without consuming the student's progress.
- **Documentation:** README was rewritten around the real evaluator workflow, Windows local-app entry point, source execution, security boundary, CI/distribution policy, and current limitations. The obsolete statement that Review was only a local draft was removed. The final commit/push are recorded only after execution.

## 2026-08-12T20:40:00+08:00 - Frozen material import release repair

- **Trigger / scope:** the student asked for a practical Edge acceptance run and explicitly lowered the closure bar to a usable, test-passing course submission. The first real release import returned `content_unreadable` for a valid UTF-8 TXT file. This bounded hotfix did not use subagents or add dependencies; the student had already waived the time-heavy Superpowers workflow for final repository handoff.
- **Root cause / RED:** frozen `extract_material()` built `[ProjectB.exe, extract_text.py, --worker, ...]`. `ProjectB.exe` runs the WebUI launcher rather than a Python script interpreter, so the child exited before writing its result. The two focused regression assertions failed exactly on the old command and missing launcher worker entry point (`2 failed`).
- **GREEN / artifact:** frozen extraction now invokes `[ProjectB.exe, --material-worker, source, output]`; the launcher dispatches that private command directly to the existing fail-closed extractor. Material plus Windows distribution tests returned `9 passed`; scoped Ruff passed; pinned PyInstaller `6.21.0` returned `WINDOWS_BUILD_PASS`. The rebuilt artifact is `29,223,634` bytes, SHA-256 `8B89717EDCA32720009750783B918130EA27A9BDE62CE159C11BE9721458047E`.
- **Real UI acceptance:** in Microsoft Edge, the release WebUI displayed the imported `os-basics.txt`, created and confirmed the `互斥` mapping against line 1, submitted the deterministic exercise, passed both `mutual_exclusion` and `witness_matches`, and generated an evidence ID. The Edge extension still requires its separate `Allow access to file URLs` permission for automated chooser control; the application import itself was verified through the rebuilt executable. The Review UI/API closure is recorded in the newer 22:20 entry above.
...
- **Second remote diagnostic:** run `31577811967` proved the pre-checkout install executed, but scanner/backend still failed at their Git-consuming steps while the other three jobs stayed green. The remaining container boundary was workspace ownership: Git rejected the host-mounted checkout as dubious ownership. A second RED (`1 failed, 22 passed`) now requires job-scoped `GIT_CONFIG_COUNT/KEY/VALUE` to trust only `${{ github.workspace }}`; GREEN returned `23 passed`, `CI_CONTRACT_PASS`, `CI_SEED_CONTRACT_PASS`, Ruff green, diff clean, and scoped credential scans green. No global Git configuration is modified.
...
- **Root cause evidence:** PyInstaller archive inspection showed about 7.4 MB of optional Pillow native extensions. A real PDF fixture returned page count 2 with all `PIL` imports blocked and `PIL_IMPORTED=False`, proving the production page-count path does not require Pillow.
- **RED / GREEN:** the strengthened Windows contract first failed `1 failed, 1 passed` because `Analysis.excludes` did not contain `PIL`. After the single configuration change, the Windows contract plus real PDF extraction suite returned `7 passed`.
- **Artifact verification:** pinned PyInstaller 6.21.0 rebuild returned `WINDOWS_BUILD_PASS`. The current artifact is `29,220,439` bytes, SHA-256 `12C6131CA956FD23C5A0C14C18F74D399E09F811F0B0DA4AF9303A851C316201`; archive inspection found no Pillow entry and development smoke returned `WINDOWS_SMOKE_PASS profile=local credential_configured=False`.
- **Full gate:** the first `scripts/test_all.py` run produced backend `281 passed / 3 failed`; all three failures were nested Git commands rejecting the sandbox-owned worktree as dubious ownership. Without changing global Git configuration, the coordinator reran the exact gate with a process-scoped `safe.directory` environment. Final result: backend `284 passed`, frontend `60 passed`, Ruff/mypy/Vite green, `CREDENTIAL_SCAN_PASS files=514`, license verification green, and `TEST_ALL_PASS mode=all`.
- **Terminal repair commit:** `b3446d1` (`perf(DIST-01): reduce frozen startup payload [agent: coordinator]`).
- **Boundary:** the retained clean VM has not measured this exact rebuilt artifact, so no `<=10.0` second, SmartScreen, or WinVault PASS is claimed. The earlier `11.487` second best result belongs only to the superseded 36.64 MB artifact.

## 2026-08-11T17:10:55+08:00 - DIST-01 VM attempt, startup repair, and DOC-01 local closure

- **Tasks / skills:** `DIST-01-VM-CLOSE`, `DOC-01`, and `REFLECTION-CLOSE`; used `executing-plans`, `systematic-debugging`, `test-driven-development`, `receiving-code-review`, `requesting-code-review`, and `verification-before-completion`. The supplied reflection draft was only lightly polished; the full brainstorming ceremony was not restarted because product design was already approved and the student explicitly requested direct editorial work. This is a documented process deviation with no product-behavior effect.
- **Startup repair TDD:** fresh worker `/root/dist01_startup_repair` first observed `openai`, then `httpx/keyring/win32ctypes`, loaded during local app assembly. REDs failed with those exact module names; GREEN defers `OpenAIAdapter`, `httpx` typing, and `WinVaultKeyring` construction. The quality reviewer requested deterministic concurrency/retry/marker-rollback coverage; controlled mutation produced `3 failed, 2 passed`, restored implementation produced `5 passed`, and the final credential file has `15 passed`.
- **Reviews:** the fresh SPEC reviewer returned Critical=0/Major=0 for the final three-file production patch. The quality reviewer found no production defect and one Important test-coverage gap; that gap was closed with five deterministic instances. A second independent quality dispatch was unavailable after the desktop update, so the coordinator re-read the final diff and reran credential, full backend, Ruff, mypy, scanner, and license gates. No Critical/Major remained; reduced reviewer independence is not represented as a fresh-agent PASS.
- **Clean VM evidence:** Windows 11 Enterprise Evaluation x64 build 26200, 2 CPUs, 8 GiB RAM, SSD-reported 80 GiB disk, and no real Python/Node/Docker runtime. Measured artifact: `36,640,080` bytes, SHA-256 `DAC2D69B27D85C65BBC32F068EA7D55AD8876237CFC177ACBB40D5F3584F09AB`. Readiness observations were `23.554`, `14.664`, and `11.487` seconds plus strict ten-second timeouts. Windows Update and Guest Control load were isolated. An AHCI attempt booted but later became unresponsive during shutdown; the exact VM process tree was identity-checked, stopped, and the VDI restored to its retained IDE attachment. `DIST-01-VM-CLOSE` remains blocked; no SmartScreen or WinVault PASS is claimed.
- **DOC / reflection TDD:** README contract first failed with `FileNotFoundError`, then passed `1 passed`; `scripts/verify_links.py` returned `LINK_VERIFICATION_PASS files=2 links=5`. The student-supplied reflection is `1938` CJK characters and contains an explicit AI-assistance declaration.
- **Current-byte verification:** `python scripts/test_all.py` with pinned Python 3.14.6 and Node 24.18.0 returned backend `284 passed`, frontend `60 passed`, Ruff/mypy/Vite green, `CREDENTIAL_SCAN_PASS files=514`, `LICENSE_VERIFICATION_PASS python=54 npm=166 direct_python=18 direct_npm=16`, and `TEST_ALL_PASS mode=all`.
- **Cleanup:** after recording root status and the worktree list, the coordinator verified that `tmp/vm-close/guest-password.txt` was the exact non-reparse disposable target and that no VBox process for the named VM was running, then removed that temporary credential file. The VM, VDI, ISO, and remaining diagnostic evidence were retained.
- **Git boundary:** main coordinator attempted ordinary `git add` with the normal worktree index. It failed at `.git/worktrees/release-v1/index.lock` with `Permission denied`. The current desktop approval policy is `never`, so no elevation request can be emitted. No alternate index, ACL change, subagent commit, or other workaround was used; commits remain pending until the student restores `Request approval` policy.
...
- **Terminal implementation commit:** `3a51c7b` (`ci(CI-01): close final pipeline bypasses [agent: coordinator]`). PLAN row 38 and the evidence manifest were changed to complete only after the staged credential scan passed.
...
- **Network deviation:** Direct Docker Hub authentication repeatedly timed out. With explicit approval, the coordinator selected a disposable `docker-container` BuildKit builder configured with `docker.m.daocloud.io` as a transport mirror. The exact reviewed Node/Python SHA-256 base digests, lock files, Dockerfile build command, and final linux/amd64 target were unchanged. The builder was removed in `finally`; no registry login or push occurred.
- **TDD / runtime RED:** The real build first failed because the Dockerfile checked nonexistent `socket.connect`; an exact contract now requires `socket.socket.connect`. Windows PowerShell 5 smoke then exposed nested Docker JSON parsing, multiline `python -c` quoting, and a stale object assertion; smoke now uses scalar `docker inspect --format` fields and stdin for the egress probe. The read-only container next proved `tempfile` could not use `/tmp`; `TMPDIR` now points to the tmpfs root, whose sticky `1777` mode remains private to the container. Restart logs proved `DemoSessionManager.close()` treated an emptied tmpfs mount root's `EROFS` as failure; the regression permits only `EROFS + existing empty root`, retaining fail-closed behavior for residual content or other errors. The first network-count RED incorrectly counted WebUI smoke traffic; the corrected probe excludes PID 1 connections whose local port is the configured service port and fails on any remaining established outbound socket.
- **GREEN / runtime receipt:** focused demo plus OCI contracts `17 passed`; exact image build passed with ID `sha256:895b7df70ba3622c91d186c8856d9e97414a94bedc2e4d626791a7340d77d234`, size `50910779`, platform `linux/amd64`, user `10001:10001`. The PLAN-exact `--rm --name projectb-demo-smoke` run produced `OCI_NETWORK_COUNT=0` and `OCI_SMOKE_PASS profile=demo user=10001:10001 readonly=true tmpfs=true`, followed by exact named cleanup. System Chrome produced `DIST02_BROWSER_PASS chrome=system desktop=1440x900 mobile=360x800 routes=import,learning,settings`.
- **Verification:** `scripts/test_all.py --all` => backend `256 passed`, frontend `60 passed`, Ruff/mypy/Vite pass, `CREDENTIAL_SCAN_PASS files=508`, license pass, `TEST_ALL_PASS mode=all`; Linux evidence and CI seed contracts pass; `git diff --check` passes. Local demo was restored after Docker released port 7860 and returns `/api/settings` 200 under PID `30884`.
- **Boundary / human decision:** public URL and deployment remain `waived` by the student; remote CI and registry push remain `not_executed`. Human changes: coordinator runtime debugging and fixes described above.
- **Terminal implementation commit:** `5145fea` (`build(DIST-02): close local OCI runtime gates [agent: coordinator]`). PLAN row 37 and the evidence manifest are updated only after the final-byte two-stage review passed with Critical=0/Major=0.
...
## 2026-08-10T18:58:00+08:00 - CI-01 coordinator final review and cleanup

- **Review:** After two read-only review rounds found verifier bypasses, the coordinator applied RED/GREEN fixes for GitHub job/step conditions, Bash `if`/`case` wrappers, PowerShell conditional blocks, scanner directness, Windows/OCI command directness, and frontend working-directory binding. A later fresh quality review found additional step-level and shell-wrapper bypasses, so CI-01 remains in progress pending the current terminal repair; no remote CI claim was made.
- **Verification:** contract tests `15 passed`; `python scripts/verify_ci_contract.py` => `CI_CONTRACT_PASS`; repository-local TEMP/TMP `scripts/test_all.py --all` => backend `269 passed`, frontend `60 passed`, Ruff/mypy/Vite/credential/license gates pass; `git diff --check` pass. `pwsh` was unavailable in that review environment, so the previously recorded same-byte `CI_SEED_CONTRACT_PASS` remained the seed receipt and was not falsely rerun.
- **Cleanup:** before cleanup `git status --short --untracked-files=all` contained only the CI-01 files listed below and `git worktree list` was checked; removed only 19 exact disposable `tmp/ci01-*` directories after the test processes ended. No checkout, root, active worktree, or user change was removed.
- **Dependency update:** DIST-02 is now complete at `5145fea`; public URL/deployment remains `waived`. CI-01 is no longer blocked by local OCI runtime and proceeds to terminal verifier repair and review.

## 2026-08-10T17:41:19+08:00 - CI-01 structural dual-CI verifier (in progress)

- **Task / skills:** `CI-01`; `test-driven-development`, `systematic-debugging`, and `verification-before-completion`. Fresh worker `/root/ci01_impl` owns the isolated `codex/ci-v1` worktree; no Luna agent and no subagent Git operation.
- **TDD:** The new `backend/tests/contracts/test_ci_files.py` first failed with the missing `scripts/verify_ci_contract.py` (4 failing tests after routing pytest to the project Python 3.14.6 toolchain). A later negative test first passed incorrectly when a backend command was wrapped in `if false`; the direct-command check was then added. Follow-up negative tests reject unapproved GitHub action refs, GitHub publication/login commands, job/step conditions, indirect and multiline-wrapped scanner/Windows/OCI commands, Bash `case` wrappers, and missing frontend working-directory setup. Final focused receipt: `15 passed`.
- **Implementation:** The verifier structurally parses the restricted YAML subset, rejects unknown jobs/duplicate keys/bypass fields, job/step conditions and unreachable shell branches, exact pins, push triggers, unit-test, direct current-suite/scanner/Windows/OCI commands, frontend working-directory drift, lock installs, explicit timeouts, and failure propagation. It emits the stable `ci-contract-v1` mapping and `CI_CONTRACT_PASS`. The verifier exposed and closed backend empty-suite fallbacks and missing job timeouts in both CI files; `ci_seed_contract.ps1` was updated accordingly.
- **Verification:** `CI_SEED_CONTRACT_PASS` (prior same-byte workflow receipt); `python scripts/verify_ci_contract.py` => `CI_CONTRACT_PASS`; contract tests => `15 passed`; repository-local TEMP/TMP `scripts/test_all.py --all` => backend `269 passed`, frontend `60 passed`, Ruff, mypy, Vite, credential and license gates pass. The default host pytest temp root remains ACL-denied; the documented repository-local temporary root is the non-bypass replacement.
- **Commit:** coordinator created `c54aafb` (`ci(CI-01): harden dual pipeline contracts [agent: coordinator]`); PLAN row 38 binds this implementation commit while terminal review fixes remain open.
...
- **TDD red:** Added `backend/tests/distribution/test_oci_contract.py`; exact bundled-Python run failed `5 failed` because OCI files and both CI jobs were absent. No image build or registry access was attempted.
- **Green/static:** Added digest-pinned multi-stage Dockerfile, demo-only entrypoint, Dockerfile-specific context exclusions, expanded SPDX plus lock/Debian inventories, strict smoke contract, evidence, and push-triggered GitHub/GitLab OCI jobs. Review found a Critical import-graph defect: the 14-package demo lock could not import unconditional PDF/OpenAI/upload dependencies. New RED isolated those packages; upload routes are now loaded only for the local upload capability, and PDF/OpenAI dependencies are lazy. The isolated demo import and 39 focused API/demo regressions passed. Follow-up review found three Major defects: restart probes treated Docker transport failure as success, GitLab used a shared container/port, and SPDX aggregated npm/Debian closures without relationships. New RED contract assertions reproduced all three. Restart now requires a running, HTTP-ready container and exact expected egress exception; GitLab names/ports are derived from `CI_JOB_ID`; deterministic SPDX generation emits 185 package records and 185 relationships. OCI plus quality-gate contract `40 passed`; PowerShell smoke parser `POWERSHELL_PARSE_PASS`; Linux dependency evidence `LINUX_EVIDENCE_PASS ci_packages=41 demo_packages=14 license_rows=41`; `CI_SEED_CONTRACT_PASS`.
- **Verification:** final static `scripts/test_all.py --all` => backend `254 passed`, frontend `60 passed`, Ruff/mypy/Vite/credential/license gates pass. Fresh SPEC and quality re-reviews both closed at Critical=0/Major=0. The quality reviewer independently confirmed 185 unique SPDX components, 185 relationships, exact npm/Python parity, no dangling relationships, fail-closed restart probes, and job-scoped GitLab image/container cleanup. The modulo-derived GitLab host port remains a non-blocking Minor for CI-01 to replace with a Docker-assigned port or serialized resource group.
- **Environment boundary:** Docker Desktop was started through approval and reported `29.1.2 linux/amd64`. Two elevated builds reached the daemon; the final attempt failed only while fetching anonymous tokens from `auth.docker.io` for both exact pinned Node/Python digests. The images were not cached locally, so `docker run`, image digest, and local-browser OCI evidence remain `not_executed`; no registry login, image push, or deployment claim is recorded.
- **Retry receipt:** At `2026-08-10T16:53:09+08:00`, the exact elevated build was retried with a 600-second timeout. It again failed before any layer build while `auth.docker.io` could not fetch anonymous tokens for the pinned Node and Python images (`connectex` timeout); runtime evidence remains unexecuted.
- **Implementation snapshot:** coordinator commit `12d709b` (`build(DIST-02): prepare governed mock image [agent: coordinator]`) preserves the reviewed static implementation. This is deliberately not the terminal task commit because the image build/run/smoke gates remain open.
- **Next:** restore access to `auth.docker.io`, rerun the exact DIST-02 build/run/smoke/inspect commands, then repeat both reviews and create the terminal task commit. Do not mark DIST-02 complete from static evidence.
- **Cleanup:** before cleanup `git status --short --untracked-files=all` was empty and `git worktree list` was recorded above. Removed only the exact disposable directories `tmp/dist02-contract`, `tmp/dist02-contract2`, `tmp/dist02-evidence-check`, `tmp/dist02-final-static`, `tmp/dist02-final-static-2`, `tmp/dist02-full`, `tmp/dist02-green`, `tmp/dist02-green2`, `tmp/dist02-ignore-green`, `tmp/dist02-ignore-red`, `tmp/dist02-import-green`, `tmp/dist02-import-red`, `tmp/dist02-major-green`, `tmp/dist02-major-green2`, `tmp/dist02-major-red`, `tmp/dist02-pytest`, `tmp/dist02-red`, `tmp/dist02-regression`, `tmp/dist02-regression2`, `tmp/dist02-review-api-temp`, `tmp/dist02-review-green`, `tmp/dist02-review-temp`, `tmp/dist02-spec-review`, `tmp/dist02-static-gate`, and `tmp/dist02-static-gate-2`. Post-cleanup `git status` remained empty and no `tmp/dist02-*` directories remained. No active worktree or repository root was touched.

## 2026-08-10T12:30:00+08:00 - DIST-01 Windows single-file distribution (coordinator closeout)

- **Task / skills:** `DIST-01`; `using-superpowers`, `executing-plans`, `test-driven-development`, `systematic-debugging`, `requesting-code-review`, `receiving-code-review`, and `verification-before-completion`. The current tool session did not expose a usable implementation dispatch turn, so `/root` implemented in the isolated `codex/distribution-v1` worktree; this process deviation is recorded rather than attributed to a worker. Fresh read-only SPEC review was dispatched to `/root/dist01_spec_review`; no Luna agent and no subagent Git operation.
- **TDD red:** Added `backend/tests/distribution/test_windows_contract.py` first. Exact bundled-Python command initially failed with `2 failed`: missing Windows packaging/evidence files and missing `windows-package` seed assertion. No dummy artifact was created.
- **Green / fixes:** Added the PyInstaller spec, launcher, keyring/PDFium hooks, fail-closed build and smoke scripts, evidence file, Windows push job, seed assertions, and worktree-aware npm resolver. SPEC review found and reproduced the fallback npm exit-code and CI command-chain risks; both were fixed with explicit `$LASTEXITCODE` checks and separate CI commands. Smoke cleanup was also regression-tested after PyInstaller one-file bootstrap children left three locked processes; cleanup now matches the exact artifact path and leaves no process/listener residue.
- **Verification:** contract `2 passed`; `CI_SEED_CONTRACT_PASS`; PyInstaller `6.21.0` build `WINDOWS_BUILD_PASS`; development smoke `WINDOWS_SMOKE_PASS profile=local credential_configured=False` with WebUI root, SQLite data root, loopback listener, disposable cleanup, and a data root containing spaces; artifact `36,634,645` bytes, SHA-256 `1E256C07B675F8C4F5B434452FED0AA9B1969E5ED932C3CCB5BC1BB9D9ABF64B`; artifact scanner `CREDENTIAL_SCAN_PASS files=0` (the canonical scanner skips `.exe` binaries); final `scripts/test_all.py --all` => backend `246 passed`, frontend `60 passed`, `TEST_ALL_PASS mode=all`, license and credential gates pass; `git diff --check` pass.
- **Scope / gates:** `DIST-01-VM-CLOSE` clean-host/SmartScreen/real WinVault lifecycle remains `not executed`; README and AC-19 final closure remain owned by `DOC-01`/VM gate. No public URL, upload, remote push, registry publication, or provider call was performed. Human changes: none.
- **Review fixes:** quality review findings for pre-existing process ownership, recursive reparse cleanup, pinned PyInstaller/Node/npm versions, and explicit frontend/CI exit checks were addressed. The review found no Critical issue; static contract checks remain complemented by the actual freezer manifest and smoke receipt. Do not mark the clean-host or README gates complete from this local receipt.
- **Commit:** implementation and test files committed by the coordinator as `662a381` (`build(DIST-01): package windows single file [agent: coordinator]`).
- **Next:** record the evidence/ledger update in a separate coordinator documentation commit. Do not mark the clean-host or README gates complete from this local receipt.
...
## 2026-08-09T18:29:36+08:00 - DEMO-01 clean-checkout 字节修复

- **触发与修复**：创建 P-02 clean worktree 后，`core.autocrlf=true` 把 manifest 绑定的 demo fixture checkout 为 CRLF，真实 RED 为 17 个 demo 测试失败。新增精确 `.gitattributes` `backend/projectb/demo/** -text`；隔离前缀 checkout 的两份 SHA-256 与 manifest 一致，DEMO suite `22 passed`，凭据/许可证门禁通过。修复提交 `a160f72ad9cd60021e6d34b2f4438978a2c2ce5e`；provider clean worktree 随后为 backend `203 passed`、`TEST_ALL_PASS mode=backend`。`Human-Changes: none`。
