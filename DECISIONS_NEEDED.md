...
> **当前快照：** 2026-07-30。产品方向仍沿用已确认的精简 v1；冷启动执行语义已修订为 SPEC `14C03D68...0713` 和 PLAN `95FF14D2...C663` 的 38-task 候选。当前字节仍需同哈希双评审和学生重新确认，G-03/G-04 未完成，项目不可派发实现。
>
> **语言约定：** 从 G-03 起，学生需要阅读、操作或签字的文档默认使用中文。命令、文件名、哈希、任务编号和工具原始输出保持原样。工程审计与机器证据不要求学生逐份阅读，可保留技术原文。

## 你现在只需要处理的门禁

1. **当前 SPEC/PLAN 确认：** 等同哈希双评审 Critical/Major 清零后，你需要重新整体确认当前两份文件；旧确认不自动继承。
2. **G-03 运行环境与条款：** 正式复测必须在 Linux/WSL2 Claude sandbox 中运行。当前环境无法枚举 WSL（`E_ACCESSDENIED`）；你需要提供或修复一个含 `pwsh`、`timeout`、`bwrap`、`socat` 的 WSL2/Linux 环境，并在运行前确认接受 Claude Code `2.1.220` README 指向的 Anthropic Commercial Terms 与 Privacy Policy。
3. **G-04：** G-03 完成并记录修订后，你需要明确回复是否批准进入实现阶段。

D-025 公网托管、远程仓库授权和学生反思都不是当前这一步需要立即完成的事项。

## G-03：异类智能体冷启动

**状态：** 正式异类尝试已发生，但任务未完成。会话 `71a50d25-4cd7-48b1-9472-8107e82779ed` 使用 `claude-sonnet-5`，报告正确哈希后只创建空目录并以空 `end_turn` 结束，费用约 `$0.4712`。会话 `32b62490-7817-4d3d-8452-7a29a4de94ea` 使用 `claude-sonnet-4-6`，读取两份文件并验证哈希后由 `ai2.1343263.xyz` 返回 `504 Gateway Time-out`，费用约 `$0.1818`。两次都没有脚本、问题、diff、规定红测、绿测或 `files=4` 自扫描，因此不能关闭 G-03。
...
**学生需要执行：** 不要再把密钥粘贴到聊天，先撤销已经暴露的 key。当前方案固定使用 `claude-sonnet-4-6` 和既定过程端点，不再自动选模型或 fallback。等新哈希双评审通过后，先确认 SPEC/PLAN，再按中文操作手册准备 WSL2/Linux sandbox；runner 会在隐藏输入中接收新的临时 key，并把总预算限制为 `$1`。原生 Windows 不会发起正式付费调用。
