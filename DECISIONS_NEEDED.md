...
Codex 桌面更新后，本任务的审批策略变成 `never`，普通 Git index 写入因 `.git/worktrees/release-v1/index.lock` 权限被拒绝，且系统不允许智能体发起提权审批。请把审批策略切回“请求批准”；主进程随后会自行申请审批并完成 Git 暂存/提交，不会把命令转交给学生。

D-025 公网托管已豁免。学生反思已提供并完成轻量润色；远程仓库、runner 和最终流水线授权仍只在准备课程提交时需要。
