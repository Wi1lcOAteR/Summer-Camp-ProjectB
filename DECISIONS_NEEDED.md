...
## D-012 — 复习计划的目标时间语义（当前人工门禁）

- **问题**：复习计划主要围绕课程考试/截止日期倒排，还是以没有固定截止日期的长期掌握为主？
- **为什么必须由你决定**：目标日期会改变 `Course.target_date` 是否必需、任务优先级、到期解释、最后冲刺行为和“成功”的时间窗口；这属于学习体验而不是可由实现便利性决定的小参数。
- **候选方案**：
  1. **推荐：可选目标日期 + 自适应安排** — 有考试/截止日期时，结合剩余时间、知识依赖和证据安排；没有日期时退化为长期复习。界面始终解释“为什么现在复习”，学生可调整日期或任务。
  2. **仅长期掌握** — 不收集考试日期，只根据历次证据安排后续提取；模型简单、适合持续学习，但可能无法帮助期末前有限时间取舍。
  3. **手动日期为主** — 学生自行设置每个任务日期，系统只做提醒与结果记录；控制感强但连续复习的自动价值较弱，管理负担高。
- **推荐方案及影响**：方案 1 同时覆盖大学课程的考试节奏与长期保持，不要求现在选择“普适最优”的间隔算法；需要持久化可选目标日期、时区、调整历史和每个任务的可见到期理由，并用可注入时钟测试。
- **不决定会阻塞**：M3 调度状态机、`Course.target_date` 语义、`ReviewTask.due_at/reason`、任务优先级、时间相关验收和后续复习界面。
