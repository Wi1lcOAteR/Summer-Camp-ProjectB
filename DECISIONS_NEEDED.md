# 需要学生决定或完成的事项

> **当前快照：** 2026-08-11。产品方向仍沿用已确认的精简 v1；本地实现、Windows/OCI 配方、CI 合同、README 和学生反思均已形成提交证据。学生已明确批准 G-04、本地交付、不做公网部署，并通过 D-026 豁免当前产物的干净 VM/十秒内部门禁。远端 CI 尚未执行。
>
> **语言约定：** 从 G-03 起，学生需要阅读、操作或签字的文档默认使用中文。命令、文件名、哈希、任务编号和工具原始输出保持原样。工程审计与机器证据不要求学生逐份阅读，可保留技术原文。

## 当前仍需学生提供的外部条件

Git 提权已由主进程完成，启动/打包修复提交为 `b3446d1`，README、反思和本地证据提交为 `9908799`。不再需要学生代执行本地 Git 命令。

D-025 已按学生确认闭合：ProjectB 是本地单文件应用，FastAPI 只在进程内提供 loopback WebUI；依据课程“如做带服务端的项目”的条件条款，不创建公网部署。D-026 当前产物干净 VM/十秒门禁也已豁免。剩余外部条件是 NJU GitLab/GitHub 仓库、runner 与最终流水线授权。

## D-026：当前 Windows 产物干净 VM 与十秒门禁（已豁免）

**状态：** 学生于 2026-08-11 明确回复“那就豁免”。上一版 36.64 MB 产物在无 Python/Node/Docker 的 Windows 11 VM 中成功启动，最低测得 `11.487` 秒；当前 29.22 MB 产物通过确定性构建、归档检查、测试和开发机 smoke。学生决定不再要求把当前产物复制进该 VM，也不再以 `<=10` 秒作为提交阻塞项。

该决定只移除内部性能/重复环境门禁，不把历史结果改写为 PASS，不放宽 Windows 单文件、loopback、凭据安全、README 限制或远端最终 CI 要求。

## G-03：异类智能体冷启动

**状态：已完成。** 两个全新 Claude Code session 只接收当前 SPEC/PLAN。Intake 返回正确哈希、两文件列表和空歧义；execution 以短检查点完成 `F-01S1A` disposable 尝试，得到规定 RED exit 1 和 GREEN exit 0，并生成 161 行 contract 与 95 行 scanner。冻结输入未改变，候选脚本不合入正式产品。完整回执、哈希、限制和旧失败历史见 `SPEC_PROCESS.md` 第 19 节。

**学生无需继续执行 G-03。** 历史 runner 已删除；后续不得把 disposable 产物冒充正式 F-01S1A。

## G-04：批准进入实现阶段

**状态：已批准。** 2026-08-03，学生明确回复“批准进入实现阶段”。前置条件已经满足：

1. 修订后的完整 SPEC/PLAN 已重新确认；
2. `writing-plans` 已生成并通过当前哈希双评审；
3. 异类编码智能体冷启动已完成；
4. 本次正式 G-03 未暴露需要改变冻结 SPEC/PLAN 的规范歧义，候选实现问题已在 disposable session 内闭合并记录。

该批准只开放本地实现、测试、评审和本地提交；远程 push、PR/MR、发布、云资源和公网部署仍须各自授权。

## D-025：公网 OCI 演示托管（本项目已豁免）

**状态：** 学生已确认本项目采用本地交付，不创建公网主机、注册表或公网 HTTPS URL。保留本地 OCI demo；该决定不阻塞本地实现、分发、CI 或 README 收口。

**已确认冲突：** 原先选择的 Hugging Face Docker Space 路线需要符合条件的付费计划，与当前“不创建付费资源”的授权冲突。

后续可选方案：

1. 使用学生或 NJU 已有的 x64 Docker 主机和现成 HTTPS；或由学生明确接受 Tailscale Funnel 的暴露边界。
2. 使用 Azure for Students 加 Azure Container Apps Consumption，但前提是学生资格有效、明确授权创建资源，并且不升级到按量付费账户。
3. 明确授权付费 Hugging Face 计划，并给出可接受的周期性费用上限。

在学生选择路线并授权对应外部操作前，不得创建账号、付款方式、云资源，不得推送镜像或部署。

## 后续外部操作授权

这些授权延后到对应 PLAN task 准备执行时分别确认：

- 创建或配置 NJU Git/GitLab 与 GitHub 仓库；
- push 分支、创建 PR/MR 或镜像提交；
- 把远程 CI 运行结果作为最终证据；
- 发布 OCI 镜像或 Windows release 产物；
- 创建云资源或部署公网 WebUI；
- 执行一次范围明确、成本受控的真实 OpenAI 集成观察。

本地实现、mock、测试和构建不自动包含这些外部权限。

## 学生本人撰写的过程证据

学生后续仍需用自己的话提供：

- 哪些 brainstorming 问题确实有帮助；
- 哪些过程感觉重复或过度形式化；
- 最重要的产品或工程取舍；
- 最终 1500--2500 个中文字符的 `REFLECTION.md`。

AI 不得代写或隐瞒 `REFLECTION.md` 的作者身份。学生初稿存在后，AI 可以校对、压缩或指出论证缺口，并必须说明辅助范围。

## 已关闭决定索引

| 编号或主题 | 已确认结果 |
| --- | --- |
| D-001 | Superpowers 已安装、启用并可调用；检测到 14 个 skills；此前已正式调用 `writing-plans` |
| D-002、D-006、D-007 | 单用户课程学习工作台；首个验收领域为操作系统并发 |
| D-008--D-010 | 本地优先的 Windows x64 WebUI，并由用户显式控制材料处理 |
| D-011--D-012 | 理解学习加连续复习与期末复习闭环 |
| D-013 | v1 使用受约束模型端口，不包含自主 agent 主循环 |
| D-015--D-018 | 单一 provider-neutral 边界、内置 OpenAI adapter、不支持任意 endpoint/plugin |
| D-020--D-023 | 确定性计划；Windows 单文件加 OCI demo；合成或有许可的公开 fixture；GitLab 主仓加 GitHub 镜像 |
| D-024 | Open Design `frontend-design` 加 `default` / Neutral Modern |
| D-026 | 当前 Windows 产物的再次干净 VM 复测与 `<=10` 秒内部指标已豁免；历史测量和限制继续保留 |
| 2026-07-25 范围收束 | 精简 v1；输入 PDF/TXT/Markdown；L+P 模式；通用概念模型，以互斥/竞态/死锁验收；延期能力已归档 |
| 2026-07-25 精简 SPEC 签字 | 学生明确确认完整 `C6231816...9AD6` 内容快照；已授权 `writing-plans`，但冷启动修订和实现批准仍是后续门禁 |

## 不需要学生决定的工程检查

依赖许可证、精确工具链物化、provider 能力/政策刷新、解析器与包兼容性、凭据扫描、干净机器构建、浏览器证据、CI schema 和性能测量都属于工程验证。检查失败可能导致后续修改 SPEC，但只有当它实际改变用户可见范围、成本或外部权限时，才会上升为学生产品决策。
