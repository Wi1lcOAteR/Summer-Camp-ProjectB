...
字母对应：D017 OpenAI/Gemini/Claude；D018 内置 adapter/受限 endpoint/任意插件；D019 期末材料范围；D020 调度预设；D021 本地技术/凭据路线；D022 公开演示；D023 远程仓库/CI；D024 Open Design。每项详细影响、推荐与阻塞范围见下文对应条目。全部选择 A 是当前工程推荐组合，但仍需学生明确回复，不能从本模板自动推定。
...
**状态：学生指出本地解析可能造成课件内容失真，要求把处理方式留给用户选择。D-009 确认首次导入必须显式选择、按课程记住且不得静默扩大外发；D-014 又确认平台应实现本地、按页/片段远端和整份 PDF/课程远端能力，具体外发由用户决定。供应商与数据政策仍未确认。**
...
- **仍未确认**：允许的供应商、区域、留存/训练政策、费用上限与凭据适配。
- **阻塞范围**：能力目录与基本清理语义已解除；provider 与数据政策仍阻塞具体适配器和凭据设计。
...
- **确认后的范围**：第一版正式提供 L（本地）、P（经确认页面/片段远端）和 F（整份 PDF/课程远端）三种模式；首次选择、每批新增文件和扩大范围均显式确认。具体 provider 另行选择。
...
- **推荐方案及影响**：推荐方案 1。它保留用户选择外发和 provider 的空间，同时让核心状态机、授权、引用、计划和删除合同保持可测试、可替换。需要下一步再决定参考 provider。
...
- **确认后的范围**：领域层只依赖统一能力、状态和错误码；无有效配置时 L 仍可用，P/F 失败关闭。非秘密 config 可包含 adapter ID、模型、受控 endpoint/区域、预算和 `credential_ref`；secret 仍须隐藏录入并进入本机安全凭据存储。更换 adapter/profile、受控 endpoint 或政策快照必须产生新授权，旧远端对象单独追踪。D-016 已进一步把首版范围限制为一个真实参考 adapter。

## D-016 — 第一版真实 Provider Adapter 交付范围（已确认）

**状态：2026-07-20 学生回复“1”，选择一个真实参考 adapter + 统一接口/完整 mock。该选择只确定真实实现数量，不选择具体供应商、模型、区域、数据政策或费用。**
...
- **推荐方案及影响**：推荐方案 1。它不替最终用户选择供应商，也不把领域层绑定到供应商；参考 adapter 只是首版经过验证的实现。具体厂商、模型、区域、政策和费用仍需在该范围确认后用官方资料收敛。
...
## D-017 — 第一版真实参考 Provider（候选资料核验中）
...
- **暂定工程推荐（不替你选择）**：OpenAI。理由是官方 File Search/Vector Stores 已核验上传、异步 ready、删除和过期入口，与现有 F 状态机最接近；页码/视觉保真缺口必须通过本地映射或失败关闭解决，不能把推荐写成无条件兼容。
- **回复格式**：`D017-A`（OpenAI）、`D017-B`（Gemini）或 `D017-C`（Claude）。如果选择 Gemini/Claude，后续 SPEC 必须先记录对 F 合同的相应修订。
- **当前安全边界**：不读取真实 key、不调用付费 API、不上传真实课件；研究只使用官方开发文档、数据政策和官方 SDK 许可证。未选择前不添加 SDK、配置字段或实现 task。
- **不决定会阻塞**：真实 adapter 名称、SDK/许可证、模型/文件端点、凭据配置、政策快照、费用上限和 P/F 联网验收；不阻塞继续完善 provider-neutral 规约。

## D-018 — Provider 配置是否允许自定义 endpoint（待批量确认）
...
- **不决定会阻塞**：config schema、SSRF 防护、能力快照、参考 adapter 的真实端到端范围。

## D-019 — 首版期末资料输入范围（待批量确认）
...
- **不决定会阻塞**：M1 材料角色、M3 期末映射、学术诚信提示、测试夹具和公开演示数据。

## D-020 — 期末调度参数预设（待批量确认）
...
- **不决定会阻塞**：M3 规则版本、任务粒度、虚拟时钟测试、期末 UI 和计划验收。

## D-021 — 本地运行与技术/凭据预设（待批量确认）
...
- **不决定会阻塞**：SPEC 技术选型、凭据后端、分发脚本、CI 构建矩阵和 localhost 安全实现。

## D-022 — 公开 WebUI 演示数据与 provider（待批量确认）
...
- **不决定会阻塞**：公开部署拓扑、README URL、演示夹具许可证、CI/CD 和凭据威胁模型。

## D-023 — 远程仓库与 CI 证据策略（待批量确认）
...
- **不决定会阻塞**：CI 文件、worktree/PR 证据、最终提交链接与最后一次 pass 记录。

## D-024 — Open Design 环境门禁（待批量确认）
...
- **不决定会阻塞**：正式 WebUI 设计系统、UI 实现和视觉验收；不阻塞继续整理非 UI 的阶段 A 文档。
