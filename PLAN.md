...
**Confirmed SPEC:** review-remediated SHA-256 `6A0DB7CAD19533FE9A31EA81A6B30ED493C01BE59D4C98DEB6EC04A89BD11E56`; confirmed product-content SHA-256 `C6231816A62C807D205FC7A3E6142C5636DE18FD8A2C580B48E63B106F959AD6`.
...
| --- | --- | --- |
| `codex/foundation-v1` | F-01S -> F-01A -> F-01B -> F-01C -> F-01D -> F-01E -> F-02 -> F-03 -> F-04 -> F-05 | next branch starts only at its reviewed terminal tip |
...
Each task is one fresh subagent session with only its task card, confirmed SPEC, predecessor interfaces, exact commands, and owned paths. The following ordered checklist is already part of every card; it is not deferred to the dispatch prompt: `S0` run preflight; then, for each comma/slash-separated failure named in that card's Red field from left to right, `R1` add exactly one assertion and its smallest fixture to the first applicable owned test path, `R2` run the card's exact Red command and record the requirement-shaped failure, `G1` add or change exactly one named production function/schema object/configuration stanza in the listed Goal path that owns that assertion, and `G2` rerun the same targeted command. Each `R1`, `R2`, `G1`, and `G2` is a separate 2--5 minute checkbox; the next named failure cannot start before the current one is green. After the last assertion, `X1` refactor one symbol, `X2` rerun targeted green, `X3` run each listed regression/gate command separately, `X4` stage only owned paths and scan, `X5` request SPEC review, `X6` request quality/security/license review, `X7` commit, and `X8` record coordinator evidence. If any one of these already-defined steps exceeds five minutes, stop as a plan defect; do not invent a new checklist during dispatch. F-01S is the sole exception to alternating contract and production code: add its eight named contract groups one group per `R1`, run after each addition while the scanner is absent, preserve the final complete-contract `CONTRACT_RED scanner_missing`, then add one scanner helper per `G1` and rerun the unchanged complete contract after each helper until green. This exception preserves contract-first TDD without combining the full contract into one step.
...
| 2 | F-01A | F-01S | no: materializes runtimes and locks | foundation | not started | none |
| 3 | F-01B | F-01A + BOOTSTRAP-LICENSE-EVIDENCE | no: closes bootstrap licenses | foundation | not started | none |
| 4 | F-01C | F-01B | no: creates frontend harness | foundation | not started | none |
| 5 | F-01D | F-01C | no: seeds push CI before feature branches | foundation | not started | none |
| 6 | F-01E | F-01D | no: creates shared quality gates | foundation | not started | none |
| 7 | F-02 | F-01E | no: migration order | foundation | not started | none |
| 8 | F-03 | F-02 | no: migration order | foundation | not started | none |
| 9 | F-04 | F-03 | no: security base | foundation | not started | none |
| 10 | F-05 | F-04 | no: security base | foundation | not started | none |
| 11 | M1-01 | F-05 | no: material contracts | m1-materials | not started | none |
| 12 | M1-02 | M1-01 | no: consumes extraction | m1-materials | not started | none |
| 13 | M1-03 | M1-02 | no: consumes material repo | m1-materials | not started | none |
| 14 | M2-01 | M1-03 | no: evaluator base | m2-learning | not started | none |
| 15 | M2-02 | M2-01 | no: registry assembly | m2-learning | not started | none |
| 16 | M2-03 | M2-02 | no: evidence authority | m2-learning | not started | none |
| 17 | M2-04 | M2-03 | no: mastery consumes evidence | m2-learning | not started | none |
| 18 | P-01 | M2-04 | no: provider port and mock | m3-api | not started | none |
| 19 | M3-01 | P-01 | no: planner contract | m3-api | not started | none |
| 20 | M3-02 | M3-01 | no: revision persistence | m3-api | not started | none |
| 21 | API-01 | M3-02 | no: creates app registry | m3-api | not started | none |
| 22 | API-02 | API-01 | no: modifies app registry | m3-api | not started | none |
| 23 | API-03 | API-02 | no: closes local API | m3-api | not started | none |
| 24 | UI-01 | API-03 | no: creates route registry | webui | not started | none |
| 25 | UI-02 | UI-01 | no: modifies route registry | webui | not started | none |
| 26 | UI-03 | UI-02 | no: modifies route registry | webui | not started | none |
| 27 | UI-04 | UI-03 | no: modifies route registry | webui | not started | none |
| 28 | UI-05 | UI-04 | no: modifies route registry | webui | not started | none |
| 29 | UI-06 | UI-05 | no: modifies route registry | webui | not started | none |
| 30 | DEMO-01 | UI-06 | no: profile assembly | demo | not started | none |
| 31 | P-02 | DEMO-01 + P-EVIDENCE | no: real adapter is isolated last | provider-openai | not started | none |
| 32 | DIST-01 | P-02 + QA-RELEASE | no: Windows release | distribution | not started | none |
| 33 | DIST-02 | DIST-01 | no: consumes frozen frontend/runtime | distribution | not started | none |
| 34 | CI-01 | DIST-02 | no: consumes all commands | release | not started | none |
| 35 | DOC-01 | CI-01 + EXT-REMOTE-PREP + D-025-HOST-CLOSE + DIST-01-VM-CLOSE | no: final verified guide | release | not started | none |
...
- **Goal / files:** Create only `scripts/tests/bootstrap_scanner_contract.ps1` and `scripts/bootstrap_scan_credentials.ps1`. The scanner helpers and G1 order are fixed: `Write-ScanRecord` (exact output/redaction), `Invoke-GitProcess` (binary-safe subprocess), `Get-WorktreeSources` (tracked paths/root safety), `Get-IndexSources` (staged path/stage-0/mode safety), `Read-WorktreeBytes`, `Read-IndexBlobBytes`, `Convert-SourceText`, `Find-DirectSecret`, `Find-AssignmentSecret`, `Find-EncodedSecret`, and `Invoke-BootstrapScan` (dedupe/sort/top-level orchestration). Each helper is one G1 and maps in order to the applicable named contract group; no additional production helper may be invented without a PLAN revision. `-Tracked` enumerates `git ls-files -z` and scans current working-tree bytes. `-Staged` enumerates `git diff --cached --name-only -z --diff-filter=ACMR`, resolves each stage-0 entry with `git ls-files --stage -z -- PATH`, rejects non-regular mode, and obtains exact index blob bytes through `Invoke-GitProcess` calling `git cat-file blob OID`; it never substitutes filesystem bytes. With both switches, scan both `(worktree,path)` and `(index,path)` source pairs even when the path is identical. No switch is `usage_missing_scope`.
...
- **Rules:** Tests assemble positive values from fragments so no complete fixture token is stored. Exact rule IDs and literal shapes are: `provider_api_key` = literal `sk-` plus 20--200 `[A-Za-z0-9_-]`; `github_token` = one literal prefix from `ghp_,gho_,ghu_,ghs_,ghr_` plus 20--255 `[A-Za-z0-9]`; `aws_access_key` = literal `AKIA` or `ASIA` plus exactly 16 `[A-Z0-9]`; `google_api_key` = literal `AIza` plus exactly 35 `[A-Za-z0-9_-]`; `slack_token` = one literal prefix from `xoxb-,xoxp-,xoxa-,xoxr-,xoxs-` plus 10--200 `[A-Za-z0-9-]`; `private_key` = `-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----`. The first five token matches require start/end or a neighbor outside `[A-Za-z0-9_-]`; the PEM expression is matched literally with the shown single ASCII spaces and five hyphens. `credential_assignment` names are case-insensitive `api_key|api-key|apikey|access_token|auth_token|client_secret|password|passwd|secret|token`, bounded on the left by start or a non-`[A-Za-z0-9_-]`, followed by optional horizontal whitespace, `:`/`=`, and optional horizontal whitespace. A quoted value uses the same opening/closing single or double quote, forbids CR/LF, permits backslash only before that matching quote or backslash, and contains 8--512 decoded characters; an unquoted value is the maximal 8--512 run of `[A-Za-z0-9_./+=:@-]`. Safe whole values after ASCII trim and case-fold are exactly `example,placeholder,changeme,not-set,none,null,redacted`, one nonempty angle-bracket pair, `\$(?:[A-Za-z_][A-Za-z0-9_]*|\{[A-Za-z_][A-Za-z0-9_]*\})`, or `\[[^\]\r\n]*redacted[^\]\r\n]*\]`. `encoded_secret` decodes exactly one layer then applies only the preceding six token/private-key rules: standard Base64 candidate `[A-Za-z0-9+/]{16,4096}={0,2}` bounded by start/end or non-`[A-Za-z0-9+/=]`; Base64URL candidate `[A-Za-z0-9_-]{16,4096}={0,2}` bounded by start/end or non-`[A-Za-z0-9_=-]`; each must have legal padding, total length divisible by four, and strict bytes-to-UTF-8 decoding. Hex candidate `[0-9A-Fa-f]{32,8192}` is even-length and bounded by start/end or non-hex. Tests name each rule ID above and assert boundary negatives for every candidate family.
...
- **Red:** create the contract first and run exactly `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/tests/bootstrap_scanner_contract.ps1`; expect exit 1 and `CONTRACT_RED scanner_missing`. The contract invokes its child scanner through the current PowerShell process executable, so the same file also runs under CI `pwsh`. A disposable PATH-first fake `git` shim deterministically emits NUL-delimited malicious/nonzero cases without a production test hook: nonzero `ls-files` proves `git_list_failed`; a staged path with zero or two stage-0 rows proves `index_entry_failed`; `../escape` proves `path_escape`; nonzero `cat-file` proves `read_failed`. A non-repository proves `git_root_failed`; modes `120000`/`160000`, a junction ancestor, a directory leaf, invalid UTF-8, unmarked NUL, and an unknown extension prove the remaining named errors. Because `scan_failed` represents only an otherwise-unclassified runtime fault, the contract uses the built-in PowerShell AST parser to prove the top-level catch emits constant `scan_failed` with exit 3 and contains no exception/value/content interpolation; both reviews must inspect that catch. The contract also covers every named rule ID, boundaries/quotes/placeholders, encodings, rename, staged-secret+clean-worktree, clean-index+dirty-worktree, both-source findings, binary-safe index blobs, exact exits, and redaction.
- **Green/refactor:** create the minimum scanner and rerun the exact Red command; expect exit 0 with exactly the named groups `usage_and_output`, `token_rules`, `assignment_quotes_boundaries`, `encodings_and_types`, `staged_vs_worktree`, `index_modes_and_rename`, `path_safety_and_errors`, and `redaction`, followed by `BOOTSTRAP_SCANNER_CONTRACT_PASS`, without changing assertions. Then initialize a fresh disposable Git repository containing only exact copies of the two owned scripts, stage both, and run the generated scanner with `-Tracked -Staged`; expect exactly `CREDENTIAL_SCAN_PASS files=4`. Every positive direct-token, assignment, private-key, and encoded fixture must therefore be assembled from non-matching fragments at runtime; a contract file that triggers its own scanner is not green even when all behavioral groups pass.
- **Done / commit:** G-03/G-03P records diff, contract red/green, and the exact clean two-file self-scan receipt, then stops without integration. Formal post-G-04 execution stages only the two files, obtains the same `files=4` index/worktree clean receipt plus both reviews, and commits `build(F-01S): add fail-closed bootstrap scanner [agent: worker]`.

### F-01A: Reproducible runtimes, manifests, and reviewed locks

- **Dependencies / parallelism:** F-01S; no, because every later task consumes these project-local runtimes and exact dependency inputs.
...
- **Goal / files:** Consume read-only `docs/engineering/PROVIDER_POLICY_V1_P_EVIDENCE.md` and `scripts/verify_provider_policy_v1.ps1`; create `backend/projectb/providers/openai_adapter.py`, `backend/projectb/providers/policy.v1.json`, and `backend/tests/providers/test_openai_adapter.py`; serially update `backend/projectb/providers/registry.py`. Evidence SHA-256 is `35a3f46e036563e3fc681df3190eb56336ab48b9d9817ad48f4d5df42230f076`, P-only and time-bounded. Mock HTTP transport tests, not real keys or paid calls, prove request/response behavior.
...
| AC-23 | F-01S/F-01A/F-01B/F-01E, all scans/reviews, DIST-01/02, DOC-01 |
