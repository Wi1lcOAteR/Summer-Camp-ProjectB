...
- **Goal / files:** Create `backend/projectb/profiles/demo.py`, `backend/projectb/demo/fixtures.manifest.json`, `backend/projectb/demo/course.json`, `backend/projectb/demo/materials.md`, `backend/tests/demo/test_demo_profile.py`, `backend/tests/demo/test_demo_security.py`, and `frontend/e2e/demo.spec.ts`; serially update `backend/projectb/profiles/registry.py`, `backend/projectb/api/app.py`, and `backend/projectb/api/routes/settings.py`. `create_app` must keep the local defaults unchanged while accepting a profile-owned HTTP policy, cookie contract, route-capability set, and published settings state; demo assembly removes upload, credential, and provider routes instead of returning a hidden or disabled implementation. Fixtures are synthetic CC0 and <=20; SQLite is ephemeral; sessions expire at 30-minute idle/two-hour absolute; upload/credential/OpenAI/cross-session state are absent and outbound calls are denied process-wide.
